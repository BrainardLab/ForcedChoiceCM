...
%    parameters. The experimenter can use key-value pairs to select whether
%    to run the forced-choice or the threshold variations of the
%    simulations, or both.
%
...
%    'runThreshold'      - Logical. If true, runs threshold simulations.
%                          Default is false.
%    'runFC'             - Logical. If true, runs forced choice simulations.
%                          Default is true.
...
%   06/08/21  dce       Edited to reflect changes to OLRayleighMatch, made
%                       options to run just one method at a time
...
p.addParameter('OD0',true,@(x)(islogical(x)));
...
p.addParameter('runThreshold',false,@(x)(islogical(x)));
p.addParameter('runFC',true,@(x)(islogical(x)));
...
% Check that at least one simulation method has been chosen
if ~(p.Results.runThreshold || p.Results.runFC)
    error('At least one simulation method must be set to true');
end

% Set up directory for saving results
outputDir = fullfile(getpref('ForcedChoiceCM','rayleighDataDir'),...
    'paramsSearchSeries',p.Results.outerFileName,subjID);
...
if isempty(p.Results.testingParamToVary2)
    ...
    coneErrFC = zeros(length(testingValsToVary),1);
    ...
    % Adjustment data arrays
    coneErrAdjust = zeros(length(testingValsToVary),1);
...
% When varying wl increment, can optionally calculate match errors using
% only the wavelengths from the coarsest match
            ...
            if strcmp(testingParamToVary,'testWlIncr')...
                ...
                if p.Results.runThreshold
                    [coneErrAdjust(i,j),matchErrAdjust(i,j),coneErrStdAdjust(i,j),...
                        ...
                        p.Results.dmac0,'S0',p.Results.S0,'lambdaMax0',...
                        p.Results.lambdaMax0,'OD0',p.Results.OD0,'coneNoise',...
                        p.Results.coneNoise,'plotTitle',[plotTitle ', Threshold'],...
                        ...
                        'outerFileName',p.Results.outerFileName,'errScalar',...
                        p.Results.errScalar,'sdLambdaMax',p.Results.sdLambdaMax,...
                        'sdDensity',p.Results.sdDensity);
                end
                if p.Results.runFC
                    [coneErrFC(i,j),matchErrFC(i,j),coneErrStdFC(i,j),...
                        ...
                        'dlens0',p.Results.dlens0,'S0',p.Results.S0,'lambdaMax0',...
                        p.Results.lambdaMax0,'OD0',p.Results.OD0,'restrictBySd',...
                        p.Results.restrictBySd,'dmac0',p.Results.dmac0,...
                        'adjustmentLength',p.Results.adjustmentLength,...
                        'sampledObservers',sampledObservers,'plotTitle',...
                        [plotTitle ', FC'],'coneNoise',...
                        p.Results.coneNoise,'noiseScaleFactor',...
                        p.Results.noiseScaleFactor,'fieldSize',...
                        p.Results.fieldSize,'errWls',errWls,'stimLimits',...
                        p.Results.stimLimits,'lambdaRef',p.Results.lambdaRef,...
                        'minimizeConeErr',p.Results.minimizeConeErr,...
            ...
            elseif strcmp(testingParamToVary,'testWlIncr')...
                ...
                if p.Results.runThreshold
                    [coneErrAdjust(i,j),matchErrAdjust(i,j),coneErrStdAdjust(i,j),...
                        ...
                        p.Results.testingValsToVary2(j),'coneNoise',...
                        p.Results.coneNoise,'errWls',errWls,...
                        'stimLimits', p.Results.stimLimits,'lambdaRef',...
                        p.Results.lambdaRef,'minimizeConeErr',...
                        p.Results.minimizeConeErr,'outerFileName',...
                        p.Results.outerFileName,'errScalar',...
                        p.Results.errScalar,'sdLambdaMax',p.Results.sdLambdaMax,...
                        'sdDensity',p.Results.sdDensity);
                end
                if p.Results.runFC
                    [coneErrFC(i,j),matchErrFC(i,j),coneErrStdFC(i,j),...
                        ...
                        p.Results.OD0,'adjustmentLength',p.Results.adjustmentLength,...
                        'sampledObservers',sampledObservers,'plotTitle',...
                        [plotTitle ', FC'],'errWls',errWls,'noiseScaleFactor',...
                        p.Results.testingValsToVary2(j),'coneNoise',...
                        p.Results.coneNoise,'fieldSize',p.Results.fieldSize,...
                        'stimLimits',p.Results.stimLimits,'lambdaRef',...
                        p.Results.lambdaRef,'minimizeConeErr',...
                        p.Results.minimizeConeErr,'outerFileName',...
                        p.Results.outerFileName,'errScalar',...
            ...
            elseif strcmp(testingParamToVary,'nMatches')...
                ...
                if p.Results.runThreshold
                    [coneErrAdjust(i,j),matchErrAdjust(i,j),coneErrStdAdjust(i,j),...
                        ...
                        'lambdaRef',p.Results.lambdaRef,'minimizeConeErr',...
                        p.Results.minimizeConeErr,'outerFileName',...
                        p.Results.outerFileName,'errScalar',...
                        p.Results.errScalar,'sdLambdaMax',p.Results.sdLambdaMax,...
                        'sdDensity',p.Results.sdDensity);
                ...
                if p.Results.runFC
                    [coneErrFC(i,j),matchErrFC(i,j),coneErrStdFC(i,j),...
                        ...
                        p.Results.LMEqualOD,'dlens0',p.Results.dlens0,'restrictBySd',...
                        p.Results.restrictBySd,'dmac0',p.Results.dmac0,...
                        'S0',p.Results.S0,'lambdaMax0',p.Results.lambdaMax0,...
                        'OD0',p.Results.OD0,'adjustmentLength',p.Results.adjustmentLength,...
                        'sampledObservers',sampledObservers,'plotTitle',...
                        [plotTitle ', FC'],'coneNoise',p.Results.coneNoise,...
                        'noiseScaleFactor',p.Results.testingValsToVary2(j),'fieldSize',...
                        p.Results.fieldSize,'stimLimits',p.Results.stimLimits,...
                        'lambdaRef',p.Results.lambdaRef,'minimizeConeErr',...
                        p.Results.minimizeConeErr,'outerFileName',p.Results.outerFileName,...
                        'errScalar', p.Results.errScalar,'sdLambdaMax',...
                        p.Results.sdLambdaMax,'sdDensity',p.Results.sdDensity);
                end
            end
            % Save filenames
        ...
        if strcmp(testingParamToVary,'testWlIncr')
            ...
            if p.Results.runThreshold
                [coneErrAdjust(i),matchErrAdjust(i),coneErrStdAdjust(i),...
                    ...
                    p.Results.noiseScaleFactor,'coneNoise',p.Results.coneNoise,...
                    'fieldSize',p.Results.fieldSize,'stimLimits',...
                    p.Results.stimLimits,'lambdaRef',p.Results.lambdaRef,...
                    'minimizeConeErr',p.Results.minimizeConeErr,...
                    'outerFileName',p.Results.outerFileName,'errScalar',...
                    p.Results.errScalar,'sdLambdaMax',p.Results.sdLambdaMax,...
            ...
            if p.Results.runFC
                [coneErrFC(i),matchErrFC(i),coneErrStdFC(i),...
                    ...
                    'minimizeConeErr',p.Results.minimizeConeErr,'coneNoise',....
                    p.Results.coneNoise,'outerFileName',p.Results.outerFileName,...
                    'errScalar',p.Results.errScalar,'sdLambdaMax',...
                    p.Results.sdLambdaMax,'sdDensity',p.Results.sdDensity);
            end
        ...
        elseif strcmp(testingParamToVary,'noise')
            ...
            if p.Results.runThreshold
                [coneErrAdjust(i),matchErrAdjust(i),coneErrStdAdjust(i),...
                    ...
                    'dmac0',p.Results.dmac0,'S0',p.Results.S0,'lambdaMax0',...
                    p.Results.lambdaMax0,'OD0',p.Results.OD0,'adjustmentLength',....
                    ...
                    'minimizeConeErr',p.Results.minimizeConeErr,'coneNoise',...
                    p.Results.coneNoise,'outerFileName',p.Results.outerFileName,...
                    'errScalar',p.Results.errScalar,'sdLambdaMax',p.Results.sdLambdaMax,...
                    'sdDensity',p.Results.sdDensity);
            end
            if p.Results.runFC
                [coneErrFC(i),matchErrFC(i),coneErrStdFC(i),matchErrStdFC(i),...
                    ...
                    'sampledObservers',sampledObservers, 'plotTitle',...
                    [plotTitle ', FC'],'coneNoise',p.Results.coneNoise,...
                    'noiseScaleFactor',testingValsToVary(i),'fieldSize',p.Results.fieldSize,...
                    'stimLimits',p.Results.stimLimits,'lambdaRef',p.Results.lambdaRef,...
                    'minimizeConeErr',p.Results.minimizeConeErr,...
                    'makeNoObserverPlots',true,'outerFileName',p.Results.outerFileName,...
                    'errScalar',p.Results.errScalar,'sdLambdaMax',...
                    p.Results.sdLambdaMax,'sdDensity',p.Results.sdDensity);
            end
        ...
        elseif strcmp(testingParamToVary,'nMatches')
            ...
            if p.Results.runFC
                [coneErrFC(i),matchErrFC(i),coneErrStdFC(i),...
                    ...
                    'stimLimits',p.Results.stimLimits,'lambdaRef',p.Results.lambdaRef,...
                    'minimizeConeErr',p.Results.minimizeConeErr,'coneNoise',...
                    p.Results.coneNoise,'makeNoObserverPlots',true,...
...
if isempty(p.Results.testingParamToVary2)% Make plots for a single variable
    ...
    if ~isempty(coneErrAdjust)
        if isempty(coneErrFC)        
            plot(xVals,coneErrStdAdjust,'c-o','LineWidth',2.5);         
            labels4 = cellstr(num2str(coneErrStdAdjust,3));
            text(xVals+dx,coneErrStdAdjust+dyHigh*coneErrLim*0.5,labels4,'Color','c');
        end
    ...
    % Legend
    if ~isempty(coneErrAdjust) && ~isempty(coneErrFC)      
        legend('Forced Choice - Standard Cones',...
            'Forced Choice - Recovered Cones','Adjustment - Recovered Cones',...
            'Location','northwest');
    elseif ~isempty(coneErrAdjust)
        legend('Adjustment - Standard Cones',...
            'Adjustment - Recovered Cones','Location','northwest');
    elseif ~isempty(coneErrFC)
        legend('Forced Choice - Standard Cones',...
            'Forced Choice - Recovered Cones','Location','northwest');
    end  
    % Edit plot
    ...
    if ~isempty(matchErrAdjust)
        if isempty(coneErrFC)        
            plot(xVals,matchErrStdAdjust,'c-o','LineWidth',2.5); 
            plot(xVals,matchErrSampledAdjust,'y-o','LineWidth',2.5);
      
            labels5 = cellstr(num2str(matchErrStdAdjust,3));
            text(xVals+dx,matchErrStdAdjust+dyHigh*matchErrLim*0.5,labels5,'Color','c');          
            labels6 = cellstr(num2str(matchErrSampledAdjust,3));
            text(xVals+dx,matchErrSampledAdjust+dyHigh*matchErrLim*0.5,labels6,'Color','y');
        end
        plot(xVals,matchErrAdjust,'b-o','LineWidth',2.5);
        labels2 = cellstr(num2str(matchErrAdjust,3));
        text(xVals+dx,matchErrAdjust+dyLow*matchErrLim,labels2,'Color','b');
    end 
    % Legend
    if ~isempty(matchErrAdjust) && ~isempty(matchErrFC)
        legend('Forced Choice - Standard Cones',...
            'Forced Choice - Simulated Cones',...
            'Forced Choice - Recovered Cones','Adjustment - Recovered Cones',...
            'Location','northwest');
    elseif ~isempty(matchErrAdjust)
        legend('Adjustment - Standard Cones','Adjustment - Simulated Cones',...
            'Adjustment - Recovered Cones','Location','northwest');
    elseif ~isempty(matchErrFC)
        legend('Forced Choice - Standard Cones','Forced Choice - Simulated Cones',...
            'Forced Choice - Recovered Cones','Location','northwest');
    end
    % Edit plot
    ...
    set(theFig,'Color',[1 1 1],'Position',[10 10 1400 800]);
    hold on;
    % How many subplots do we need? This depends on whether we ran
    % simulations with one or two methods
    if isempty(coneErrFC) || isempty(coneErrAdjust)
        rowsNum = 1;
    else
        rowsNum = 2;
    end 
    ...
    if ~isempty(coneErrFC)
        % Plot forced choice data in the first two subplots
        subplot('Position', subplotPosVectors(1,1).v);
        theData = coneErrFC;
        ...
        % Plot adjustment data in the second two subplots, if it exists
        if ~isempty(coneErrAdjust)
            subplot('Position', subplotPosVectors(2,1).v);
        ...
        % Plot adjustment data in the first two subplots, if there is no forced choice data
        subplot('Position', subplotPosVectors(1,1).v);
        theData = coneErrAdjust;
        heatmap(xVals,yVals,theData,'ColorLimits',[0 coneErrLim]);
        title('Cone Error - Threshold');
