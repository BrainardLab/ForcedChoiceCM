...
%    'S0'                -Logical. If true, the parameter search constrains
%                         S cone parameters to 0. Default is true.
%    'lambdaMax0'        -Logical. If true, the parameter search constrains
%                         lambda max parameters to 0. Default is false.
%    'OD0'               -Logical. If true, the parameter search constrains
%                         optical density parameters to 0. Default is false.
...
outputDir = fullfile(getpref('ForcedChoiceCM','rayleighDataDir'),...
    'paramsSearch',p.Results.outerFileName,subjID);
...
for i = 1:nObservers
    ...
    [testSpds,primarySpds,testIntensitiesSimObs,primaryRatiosSimObs] = ...
        ...
        'testScale',p.Results.testScale,'monochromatic',p.Results.monochromatic,...
        'coneNoise',p.Results.coneNoise,'nObserverMatches',...
        p.Results.nObserverMatches,'nReversals',p.Results.nReversals,...
        'nBelowThreshold',p.Results.nBelowThreshold,'thresholdScaleFactor',...
        p.Results.thresholdScaleFactor,'rayleighPlots',false,'saveResults',...
        false,'adjustmentLength',p.Results.adjustmentLength,'noiseScaleFactor',...
        p.Results.noiseScaleFactor,'averageSpds',false,'stimLimits',...
        p.Results.stimLimits,'lambdaRef',p.Results.lambdaRef,...
        'outerFileName',p.Results.outerFileName);
    ...
    matchErr(i) = findMatchError(calcParams,stdObs,testSpdsSelected,...
        ...
        'findConeErr',p.Results.minimizeConeErr)/matchErrScalar;
    matchStandardErr(i) = findMatchError(zeros(1,8),stdObs,testSpdsSelected,...
        primarySpdsSelected,'errScalar',matchErrScalar,...
        'S',p.Results.S,'findConeErr',p.Results.minimizeConeErr)/matchErrScalar;
...
%         'lambdaRef',p.Results.lambdaRef,'adjustmentLength',...
%         p.Results.adjustmentLength,'outerFilename',p.Results.outerFilename);
...
save(fullfile(outputDir,[subjID '_paramsSearchData']));
