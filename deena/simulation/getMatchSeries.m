...
%                         Not applicable for OLRayleighMatch simulations. 
%                         Default is false.
...
%                            layer of hierarchy in the file saving tree of
...
%                       wavelength
%   06/07/21  dce       Edited to reflect changes to OLRayleighMatch
...
outputDir = fullfile(getpref('ForcedChoiceCM','rayleighDataDir'),...
    'matchFiles',p.Results.outerFileName,subjID);
...
for i = 1:nCombos
    ...
    if strcmp(method,'threshold')  % Use OLRayleighMatch threshold
        % Run simulation
        OLRayleighMatch(subjID,i,'simObserver',true,'adjustment',...
            true,'observerParams',observerParams,'fieldSize',...
            p.Results.fieldSize,'p1',lightCombos(i,1),'p2',...
            ...
            'plotResponses',p.Results.rayleighPlots,'savePlots',...
            p.Results.rayleighPlots,'adjustmentLength',...
            p.Results.adjustmentLength,'opponentParams',opponentParams,...
            'noiseScaleFactor',p.Results.noiseScaleFactor,'coneNoise',...
            p.Results.coneNoise,'stimLimits',trialStimLimits,'lambdaRef',...
            p.Results.lambdaRef,'silent',true,'outerFileName',...
            p.Results.outerFileName,'interleaveStaircases',false);
        ...
        [testSpd,primarySpd,testIntensity,primaryRatio] =...
            getMatchData(simFilePath,'averageSpds',p.Results.averageSpds);
    ...
    elseif strcmp(method,'forcedChoice')% Use OLRayleighMatch forced choice
        OLRayleighMatch(subjID,i,'simObserver',true,'observerParams',...
            observerParams,'fieldSize',p.Results.fieldSize,'p1',...
            ...
            'p1Scale',lightScaleFactors(i,1),'plotResponses',p.Results.rayleighPlots,...
            ...
            'coneNoise',p.Results.coneNoise,'stimLimits',trialStimLimits,'lambdaRef',...
            p.Results.lambdaRef,'adjustment',false,'silent',true,...
            'outerFileName',p.Results.outerFileName,'interleaveStaircases',false);
        % Extract spds from the data file
        simFile = [subjID,'_',num2str(i),'.mat'];
        simFilePath = fullfile(outputDir,simFile);
        [testSpd,primarySpd,testIntensity,primaryRatio] =...
        ...
        if p.Results.monochromatic
            ...
            if exist(p1File,'file')
                ...
                p1Spd = makeOLRayleighPrimary(lightCombos(i,1),'scaleFactor',...
                    lightScaleFactors(i,1),'nominal',false,...
            ...
            if exist(p2File,'file')
                ...
                p2Spd = makeOLRayleighPrimary(lightCombos(i,2),'scaleFactor',...
                    lightScaleFactors(i,2),'nominal',false,'subtractDark',true);
            end
            if exist(testFile,'file')
                ...
                tSpd = makeOLRayleighPrimary(lightCombos(i,3),'scaleFactor',...
                    lightScaleFactors(i,3),'nominal',false,'subtractDark',true);
            end
            ...
            [testSpd,primarySpd,testIntensity,primaryRatio] =...
                computePredictedRayleighMatch(p1Spd,p2Spd,tSpd,...
                observer,'S',S,'addDarkSpd',true,'darkSpd',darkSpd);
        end
    end
