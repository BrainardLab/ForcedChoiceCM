...
[metamerPrimary,metamerSpd,metamerLMS] = FindMetamer(stimulusParams.matchApparatusParams,T,T*referenceSpd);
...
opponentDirLMS = OpponentContrastToLMS(observerParams1.colorDiffParams,T*metamerSpd,opponentDirContrast);
[opponentDirPrimary,opponentDirSpd,opponentDirLMSCheck] = FindMetamer(stimulusParams.matchApparatusParams,T,opponentDirLMS);
if (max(abs(opponentDirLMS-opponentDirLMSCheck)) > 1e-8)
    error('Do not recover metameric cone excitations properly');
end
[opponentDirDiff,opponentDirContrastCheck] = ComputeMatchDiff(observerParams1.colorDiffParams,T*adaptationSpd,T*referenceSpd,opponentDirLMS);
